...
网络面板和表格面板是可固定的选项卡式面板。你可以通过单击 CytoPanel 右上角的浮动窗口控件 ![](images/quick-tour-of-cytoscape/float-window.png) 来取消停放任何面板。这在当你想为网络面板分配尽可能多的屏幕空间时很有用。现在有两个 Cytoscape 窗口，一个主窗口和一个标题为 Table Panel 的新窗口，类似于下面。当你将鼠标指针放在单元格上时，将显示一个弹出指示。
...
编辑菜单包含剪切、复制和粘贴，以及撤销和重做功能，可以用于表格面板、网络编辑器和布局中所做的编辑。
...
在 `Edit -> Preferences -> Properties...` 下可以找到属性和应用程序的编辑首选项。有关如何编辑首选项的更多详细信息，请参见[此处](/cytoscape-preferences#cytoscape-首选项)。

![](images/quick-tour-of-cytoscape/edit-menu.png)

### 视图

视图菜单可以显示或隐藏面板，包括 Control Panel，Table Panel，Result Panel，Tool Panel 和 Automation Panel。此外，它还提供了一种查看 Starter Panel 的方法。它还提供对其他与视图相关功能的控制。
...
工具菜单包含了 `cyCharts`，[`Network Analyzer`](/network-analyzer/)，[`Cytoscape Web Browser`](/cytoscape-web-browser/)，[`Network Merge`](/merge/)，[`Execute Command File...`](/automation-panel/)，`Job Status Monitor`，`Run Script File...` 和 [`Diffuse`](/finding-and-filtering-nodes-and-edges/#扩散) 等高级功能。
